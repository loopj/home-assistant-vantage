...
class VantageRGBLight(VantageEntity[RGBLoadTypes], LightEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        if ATTR_RGBW_COLOR in kwargs:
            ...
            rgbw_brightness: int | None = kwargs.get(ATTR_BRIGHTNESS)
            if rgbw_brightness is not None:
                rgbw = scale_color_brightness(rgbw, rgbw_brightness)
            ...
            # Scale the brightness of the color if provided
            rgb_brightness: int | None = kwargs.get(ATTR_BRIGHTNESS)
            if rgb_brightness is not None:
                rgb = scale_color_brightness(rgb, rgb_brightness)
            ...
            if ATTR_COLOR_TEMP_KELVIN in kwargs:
                ...
                await self.async_request_call(self.obj.set_temperature(color_temp))
